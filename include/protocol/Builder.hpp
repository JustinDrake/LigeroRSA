...
public:
	...
	ConstraintSystem<FieldT> compile(std::vector<std::vector<FieldT>>& values, bool populate = true);
	ConstraintSystem<FieldT> build_system(weightedUnionFind& representative, const std::vector<bool>& removable);
...
private:
	...
	void rewire_transformation(const size_t idx, const std::vector<bool>& removable);
	...
	std::vector<std::vector<FieldT>> compute_proof_values(
			...
			weightedUnionFind& representative);
};
...
// is an element in a sorted vector
template <class T>
...
void builder<FieldT>::determine_removable(vector<bool>& removable) const {
	...
	for(size_t cons_idx = 0; cons_idx < csz; ++cons_idx) {
		...
		if(!equality_graph[cons_idx].empty() || !in_sorted_vector(special_blocks, cons_idx)) {
		...
		switch(cons->type) {
			...
			{
				removable[cons_idx] = false;
				const inverse_constraint<FieldT>* i_cons = static_cast<const inverse_constraint<FieldT>*>(cons);
			...
			{
				const linear_constraint<FieldT>* l_cons = static_cast<const linear_constraint<FieldT>*>(cons);
...
void builder<FieldT>::rewire_quadratic(size_t idx, const vector<bool>& removable) {
	...
	for(size_t pos = 0; pos < cons->left_block.size(); ++pos) {
		...
		if(remove_left && remove_right) {
			...
			for(size_t lpos =  0; lpos < l_lcons->block.size(); ++lpos) {
				const FieldT lscalar = l_lcons->scalar[lpos];
				new_data.emplace_back(l_lcons->block[lpos], old_right_block, lscalar * old_scalar);
			}
		} else {
	...
	// make flip left and right in new_data so that left < right
	// this is done to better reduce duplicates
...
template<typename FieldT>
void builder<FieldT>::rewire_transformation(const size_t idx, const vector<bool>& removable) {
	...
	// we want to be able to look up all (source, pos, scalar) by [target, target_position],
	// where 'target' is anything we map FROM
	...
	for(size_t pos = 0; pos < cons->block.size(); ++pos) {
		...
		if(removable[old_block] && sub_cons->type == ConstraintType::linear) {
			...
			for(size_t sub_pos = 0; sub_pos < sub_lcons->block.size(); ++sub_pos) {
				...
				for(size_t loc = 0; loc < block_size; ++loc) {
					lookup[make_pair(old_block, loc)].emplace_back(sub_block, loc, sub_scalar);
				}
			}
		} else if(removable[old_block] && sub_cons->type == ConstraintType::transformation) {
			if(seen_blocks.count(old_block)) continue;
			seen_blocks.insert(old_block);
			...
			for(size_t sub_pos = 0; sub_pos < sub_tcons->block.size(); ++sub_pos) {
				const size_t sub_block =
					sub_tcons->block[sub_pos];
				const size_t sub_source_position =
					sub_tcons->source_position[sub_pos];
				const size_t sub_target_position =
					sub_tcons->target_position[sub_pos];
				const FieldT& sub_scalar =
					sub_tcons->scalar[sub_pos];
	...
	for(size_t pos = 0; pos < cons->block.size(); ++pos) {
		...
		if(seen_blocks.count(old_block)) {
			...
			for(const auto& entry : it->second) {
				new_data.emplace_back(std::get<0>(entry),
						std::get<1>(entry),
						old_target_position,
						old_scalar * std::get<2>(entry));
			}
		} else {
			new_data.emplace_back(old_block, old_source_position, old_target_position, old_scalar);
		}
	}
	...
	// this is is so that when we later apply randomness, if a transformation pulls
	// from ITSELF (as it might!) it will only add scalars to locations which
	// have already adjusted all scalars of elements which map into it under the
	// transformation
	std::sort(new_data.begin(), new_data.end(),
			[](const data& a, const data& b) -> bool { 
...
template<typename FieldT>
void builder<FieldT>::rewire_linear(const size_t idx, const vector<bool>& removable) {
	linear_constraint<FieldT>* const cons = static_cast<linear_constraint<FieldT>*>(constraints[idx]);
	// before anything, we check to see if it depends on any removable transformation constraints
	// if so, we simply convert it to a transform and call that rewire function
	for(size_t pos = 0; pos < cons->block.size(); ++pos) {
		const size_t old_block = cons->block[pos];
		const constraint* sub_cons = constraints[old_block];
		if(removable[old_block] && sub_cons->type == ConstraintType::transformation) {
...
vector<vector<FieldT>> builder<FieldT>::compute_proof_values(
	...
	auto get_block_vals = [&](size_t idx) -> const vector<FieldT>& {
			...
			if(constraints[idx]->type == ConstraintType::constant) {
				return static_cast<const constant_constraint<FieldT>*>(constraints[idx])->value;
			} else {
	...
	for(size_t idx = 0; idx < constraints.size(); ++idx) {
		...
		if(representative.id(idx) != idx) {
			proof_values_location[idx] = proof_values_location[representative.id(idx)];
		...
		switch(constraints[idx]->type) {
			...
			case ConstraintType::transformation:
			{
				const transformation_constraint<FieldT>* const cons = 
					static_cast<const transformation_constraint<FieldT>*>(constraints[idx]);

				vector<FieldT> ans(block_size, FieldT(0));
				for(size_t j = 0; j < cons->block.size(); ++j) {
					ans[cons->target_position[j]] += (get_block_vals(cons->block[j])[cons->source_position[j]] * cons->scalar[j]);
				}
...
// constant takes precedence, else first thing
template<typename FieldT>
weightedUnionFind builder<FieldT>::calc_equivalence_classes() {
	// use the weighted union find structure to determine the correct representative
...
ConstraintSystem<FieldT> builder<FieldT>::build_system(weightedUnionFind& representative, const vector<bool>& removable) {
	...
	for(size_t idx = 0; idx < constraints.size(); ++idx) {
		...
		switch(cons->type) {
			...
			case ConstraintType::quadratic:
				{
					quadratic_constraint<FieldT>* c = static_cast<quadratic_constraint<FieldT>*>(cons);
...
// +-----------------------+
template<typename FieldT>
ConstraintSystem<FieldT> builder<FieldT>::compile(vector<vector<FieldT>>& values, bool populate) {
