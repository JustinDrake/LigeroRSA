...
const auto total_timeout            = 20min;
constexpr uint64_t layeringDelayUs  = 50000ull;

// Exporting to S3?
const bool s3export = true;
...
namespace ligero
{
    namespace GatheringCoordinator
    {
        ...
        void generatePublicData(
                ...
                while (!goodAlloc) { 
                    ...
                    try {
                        ...
                        std::cerr << "bad_alloc caught, prop detection: " << ba.what() << " client = " << id << '\n';
                        goodAlloc = false;
                    }
                }
            ...
            size_t numVerifiers = 2)
        {
            ...
            auto runIdentifier = boost::uuids::to_string(boost::uuids::random_generator()());
            ...
            ZeroMQGatheringTransport verificationTransport(std::string("tcp://") + std::string(ip) + std::string(":") + std::string(port), numVerifiers);
            LOG(INFO) << "Gathering connecting " << std::string("tcp://") + std::string(ip) + std::string(":") + std::string(port);
            ...
            if (testing) {
                ...
                for (auto& partyId : transport.ids) {
                    ...
                    if (timeRemaining < std::chrono::milliseconds(0)) timeRemaining = std::chrono::milliseconds(0);
                    auto maybeResponse = verificationTransport.awaitShortRegistration(MessageType::VERIFIER_READY, timeRemaining);

                    // tick tock
                    auto timeSpent = std::chrono::steady_clock::now() - startTime;
                    timeRemaining -= std::chrono::duration_cast<std::chrono::milliseconds>(timeSpent);
                    totalTimeRemaining -= std::chrono::duration_cast<std::chrono::milliseconds>(timeSpent);
                    ...
                    if (!hasError(maybeResponse)) {
                        ...
                        LOG(INFO) << "Allocating Verifier " << verifierId << " with IP " << verifierIPAddress << " to Party " << partyId;
                        }
                    else {
            ...
            std::unordered_map<SocketId, PublicData*, boost::hash<SocketId>> publicDataValues;
            std::unordered_map<SocketId, std::string, boost::hash<SocketId>> publicDataMessages;
            ...
            // ========================================================================================================================================
            {
                for (auto& id : transport.ids) {
                    ...
                    auto startTime = std::chrono::steady_clock::now();
                    if (totalTimeRemaining < std::chrono::milliseconds(0)) totalTimeRemaining = std::chrono::milliseconds(0);
                    auto responseMaybe = transport.awaitNextInput(totalTimeRemaining);  // MessageType::GATHER_SIGMA_DATA
                    
                    auto timeSpent = std::chrono::steady_clock::now() - startTime;
                    totalTimeRemaining -= std::chrono::duration_cast<std::chrono::milliseconds>(timeSpent);

                    if (hasError(responseMaybe)) {
                        // one or more parties is timed out, kick out and restart the protocol again
                        std::vector<SocketId> goodPartyList(goodParties.begin(), goodParties.end());
                        ...
                        verificationTransport.broadcast(MessageType::PROTOCOL_RESTART, goodVerifierList);
                        ...
                        if (!testing) {
                            ...
                            if (std::find(failedVerifier.begin(), failedVerifier.end(), socketId) == failedVerifier.end()) {
                                LOG(INFO) << "Send SigmaData to Verifier (" << taskAllocation[socketId] << " for Party " << socketId;
                                verificationTransport.sendRaw(taskAllocation[socketId], sigmaProtocolPublicData);
                            }
                            else {
                                LOG(INFO) << "Send " << boost::uuids::to_string(socketId) << "'s sigma protocol data to local disk";
                                std::string sigmaDataFileName = filename + "-" + runIdentifier + "-" + boost::uuids::to_string(socketId) + ".sigmadata";
            ...
            // ========================================================================================================================================
            {
                if (testing) {
                    ...
                    {
                        {
                            std::string timerVerificationPublicData = std::string("3.a. publicdata, public");
                            ...
                            for (size_t i = 0; i < ids.size(); i++) {
                                ...
                                threads.emplace_back(generatePublicData<T, Degree, NbPrimesP, NbPrimesQ>,
                                        std::ref(encryptedCoordinator),
                                        std::ref(ids[i]),
                        ...
                        {
                            std::string timerVerificationSerialization = std::string("3.a. publicdata, serialization");
                            ...
                            for (size_t i = 0; i < ids.size(); i++) {
                                threads.emplace_back(serializePublicData,
                                        std::ref(ids[i]),
                                        std::ref(publicDataValues[ids[i]]),
                        ...
                        {
                            // send them all
                            for (auto it = publicDataMessages.begin(); it != publicDataMessages.end(); ++it) {
...
/*
                                    ...
                                    std::string pdFileName = filename + "-" + runIdentifier + "-" + boost::uuids::to_string(sid) + ".puiblicdata";
                                    sendToFile(it->second, pdFileName);

                                    if (s3export) {
                                        std::string awsCommand = "aws s3 cp " + pdFileName + " s3://" + bucketName + "/" + pdFileName;
...
*/
                                    ...
                                    LOG(INFO) << "Sending serialized public data for party " << it->first << " to verifier " << taskAllocation[it->first]; 
                                    verificationTransport.sendMsg(taskAllocation[it->first], it->second);
                                }                        
                            }
                        }
                    }
            ...
            do {
                ...
                auto startTime = std::chrono::steady_clock::now();
                if (totalTimeRemaining < std::chrono::milliseconds(0)) totalTimeRemaining = std::chrono::milliseconds(0);
                auto responseMaybe = transport.awaitNextInput(totalTimeRemaining);   // MessageType::GATHER_PROOFS
                ...
                // No error happened
                    ...
                    if (!testing) {
                        ...
                        if (it == ids.end()) {
                            LOG(INFO) << "Receiving from unknown party " << boost::uuids::to_string(socketId);
                            continue;
                        }
...
/*                             complete = true;
                            for (auto id: ids) {
                                ...
                                if (it!=ids.end()) {
                                    int socketIndex = std::distance<SocketIterator>(ids.begin(), it);
                                    if (proofsReceived[socketIndex]<numberOfProofs) {
                                        complete = false;
                                        break;
                                    }
                                }
...
*/
                        complete = std::all_of(proofsReceived.begin(), proofsReceived.end(), [](size_t received) { return received == 21; });


                        {
                            // Verifier is not avaliable
                            if (failedVerifier.find(socketId) != failedVerifier.end()) {
                                //auto proofIndex = proofsReceived[SocketIndex] - 1;
                                //sendToFileRaw(argumentOfKnowledge, filename + "-" + runIdentifier + "-" + std::to_string(socketId) + ".proof" + std::to_string(proofIndex));

                                // simply wait and save all proofs later
                            }
                            else {
                                LOG(INFO) << "Send Proof for Modulus " << proofsReceived[socketIndex]-1 << " to Verifier (" << taskAllocation[socketId] << " for Party " << socketId;
                                verificationTransport.sendRaw(taskAllocation[socketId], argumentOfKnowledge);
                            }
                            ...
                            printAtXY(1,4,std::string("verifying proof:") + proofNb + std::string("_") + ssocketId + std::string("_") + proofNb);
                            // sendToFileRaw(argumentOfKnowledge, filename + stagProof + ssocketId + std::string("_") + proofNb);
                        }
                    }
                }
            } while (!complete);
            ...
            std::vector<bool> verifierFailed(nbParties, true); // We are deemed to have failed unless and until we successfully verify

            verificationTransport.broadcast(MessageType::GATHER_REPORTS, verificationTransport.ids);
            ...
            if (!testing) {
                ...
                for (size_t idx = 0 ; idx < verificationSize; idx++) {
                    LOG(INFO) << "Receiving Verifier Report";
                    printAtXY(1,1+firstline,std::string("Verifiers Reports: (")+std::to_string(verificationSize) + std::string(" parties)"));
                    printAtXY(1,2+firstline,std::string("=================="));
                    printAtXY(1,3+firstline,std::string("Moduli:"));
                    for (size_t i = 0; i < numberOfProofs;i++) {
                        printAtXY(i*displayStride+displayOffset,3+firstline,std::to_string(i));
                    }

                    printAtXY(1,4+firstline,std::string("Successful Verifications:"));

                    for (size_t i = 0; i < numberOfProofs;i++) {
                        printAtXY(i*displayStride+displayOffset,4+firstline,std::to_string(0));
                    }
                    ...
                    auto startTime = std::chrono::steady_clock::now();
                    if (totalTimeRemaining < std::chrono::milliseconds(0)) totalTimeRemaining = std::chrono::milliseconds(0);
                    auto maybeResponse = verificationTransport.awaitNextInput(totalTimeRemaining);  // MessageType::VERIFIER_REPORT
                    ...
                    if (hasError(maybeResponse)) {
                        ...
                        auto it = std::find_if(ids.begin(), ids.end(), [&](const auto& id) { return taskAllocation[id] == response.first; });
                        ...
                        LOG(INFO) << "Report received from Verifier " << response.first << " was for Party " << ids[idx];
                        ...
                        verifierFailed[socketIndex] = false;
                        LOG(INFO) << "Verifier " << response.first << " reporting for party " << ids[socketIndex];

                        auto verifySuccess = std::all_of(report.begin(), report.end(), [](int x) { return x; });
                        if (verifySuccess) {
                            goodParties.insert(ids[idx]);
                        }
                        else {
                            LOG(INFO) << "Verification for party " << ids[idx] << " failed. Adding to kickout list";
                        }
                        ...
                        std::string msg("");
                        for (size_t i = 0; i < numberOfProofs; i++) {
                            msg += std::to_string(i) + std::string(": ") + std::to_string(proofsSuccess[i]);
                        ...
                        for (size_t i = 0; i < numberOfProofs;i++) {
                            printAtXY(i*displayStride+displayOffset,4+firstline,std::to_string(proofsSuccess[i]));
                        }
                    } 
                }

                auto reportReceiptNb = std::count(verifierFailed.begin(), verifierFailed.end(), false);
                LOG(INFO) << "Successfully received report from " << reportReceiptNb << " verifiers";
                ...
                if ((goodParties.size() != reportReceiptNb)) {
                    ...
                    std::transform(goodParties.begin(), goodParties.end(), std::back_inserter(restartVerifiers), [&](const auto& id) { return taskAllocation[id]; });
                    verificationTransport.broadcast(MessageType::PROTOCOL_RESTART, restartVerifiers);
                    verificationTransport.ids.clear();
                    ...
                    verificationTransport.broadcast(MessageType::END, verificationTransport.ids);
                }

                // Save failed sigma data to local disk
                for (const std::string& file : sigmaDataFileNames) {
                    LOG(INFO) << "Pushing " << file << " AWS";
                    std::string awsCommand = "aws s3 cp " + file + " s3://" + bucketName + "/" + file + std::string(" > s3_upload.log 2>&1") ;
                    system(awsCommand.c_str());
                }
                ...
                for (size_t i = 0; i < ids.size(); i++) {
                    if (verifierFailed[i]) {
                        ...
                        auto it = publicDataMessages.find(ids[i]);
                        
                        if (it != publicDataMessages.end()) {
                            LOG(INFO) << "Sending public data to file";
                            std::string pdFileName = filename + "-" + runIdentifier + "-" + boost::uuids::to_string(ids[i]) + ".publicdata";
                            sendToFile(*it, pdFileName);

                            if (s3export) {
                                std::string awsCommand = "aws s3 cp " + pdFileName + " s3://" + bucketName + "/" + pdFileName + std::string(" > s3_upload.log 2>&1") ;
                                system(awsCommand.c_str());
                            }
                        }                        

                        for (size_t j = 0; j < proofs[i].size(); j++) {
                            std::string proofFileName = filename + "-" + runIdentifier + "-" + boost::uuids::to_string(ids[i]) + ".proof." + std::to_string(j);
                            LOG(INFO) << "Saving proofs " << j << "to disk";
                            sendToFileRaw(proofs[i][j], proofFileName);

                            if (s3export) {                            
                                std::string awsCommand = "aws s3 cp " + proofFileName + " s3://" + bucketName + "/" + proofFileName + std::string(" > s3_upload.log 2>&1");
