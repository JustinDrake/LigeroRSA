...
class NTT {
 protected:
  uint64_t *phis, *shoupphis, *invpoly_times_invphis,
      *shoupinvpoly_times_invphis, *omegas, *shoupomegas, *invomegas,
      *shoupinvomegas, invpolyDegree;

  size_t _J, _indexModulus, _degree;
  ...
  struct ntt_loop_body {
   public:
    ...
    ntt_loop_body(uint64_t const p) { _p = p; }

    inline void operator()(uint64_t *x0, uint64_t *x1, uint64_t const *winvtab,
                           uint64_t const *wtab) const {
      uint64_t u0 = *x0;
      uint64_t u1 = *x1;
      ...
      uint64_t q = ((__uint128_t)t1 * (*winvtab)) >>
                   params<uint64_t>::kModulusRepresentationBitsize;
  ...
  static constexpr uint64_t getModulus(size_t n) {
    return params<uint64_t>::P[n];
  }

  /** Performs the bulk of the NTT
   * @param x coefficients of the polynomial
   * @param wtab table fo roots of unity
   * @param winvtab modified table
   * @param p modulus p (computations in Fp)
   * @return polynomial degree/4
   */
  size_t run(uint64_t *x, const uint64_t *&wtab, const uint64_t *&winvtab,
             const uint64_t p) {
    ...
    for (size_t w = 0; w < this->_J; w++) {
      ...
      for (size_t r = 0; r < M; r++) {
        for (size_t i = 0; i < N / 2; i += 2) {
          body(&x[N * r + i + 0], &x[N * r + i + 0 + N / 2], &winvtab[i + 0],
               &wtab[i + 0]);
          body(&x[N * r + i + 1], &x[N * r + i + 1 + N / 2], &winvtab[i + 1],
               &wtab[i + 1]);
        }
      }
      wtab += N / 2;
  ...
  /** NTTvalues evaluates a given polynomial on the evaluation points (_phis)
   * @param x values of polynomial on evaluation points
   * @param wtab table of roots of unity
   * @param winvtab modified table
   * @param p modulus p (computations in Fp)
   * @return boolean, true if computation was successful
   */
  bool ntt(uint64_t *x, const uint64_t *wtab, const uint64_t *winvtab,
           uint64_t const p) {
    ...
    for (size_t r = 0; r < M; r++, x += 4) {
      ...
      uint64_t q = ((__uint128_t)t * winvtab[1]) >>
                   params<uint64_t>::kModulusRepresentationBitsize;
   ...
   * @param x values of polynomial on evaluation points
   * @param inv_wtab table of inverse roots of unity
   * @param inv_winvtab modified inverse table
   * @param invK inverse polynomial degree
   * @param p modulus p (computations in Fp)
   * @return boolean, true if computation was successful
   */
  bool inv_ntt(uint64_t *x, const uint64_t *const inv_wtab,
               const uint64_t *const inv_winvtab, const uint64_t invK,
               uint64_t const p) {
   ...
   * @param wtab table of roots of unity
   * @param wtabshoup modified table
   * @param w omega
   * @param cm the modulus p (computations in Fp)
   */
  inline void prep_wtab(uint64_t *wtab, uint64_t *wtabshoup, uint64_t w,
                        size_t cm) {
 ...
 public:
  ...
  /** Constructor for the Number-Theoretic Transformation class
   * @param indexModulus index of the modulus p (computations performed in Fp)
   * @param degree degree of the polynomial/number of evaluation points
   * @param compute templated implementation of bit-reversal
   */
  NTT(size_t indexModulus, size_t degree,
      void (*compute)(uint64_t *, uint64_t const *))
      : _compute(compute) {
    /** allocate all */
    phis = new uint64_t[degree], shoupphis = new uint64_t[degree],
    invpoly_times_invphis = new uint64_t[degree],
    shoupinvpoly_times_invphis = new uint64_t[degree],
    omegas = new uint64_t[degree * 2], invomegas = new uint64_t[degree * 2],
    shoupomegas;
    ...
    for (unsigned int i = 0;
         i <
         static_log2<params<uint64_t>::kMaxPolyDegree>::value - log2(degree);
         i++) {
    ...
    /** For the omegas it is easy, we just use the function of David Harvey
  ...
  /** Performs an in-place inverse number theoretic transform
   * @param data value of the underlying polynomial in the evaluation domain
   * (_phis for NFLlib)
   */
  inline void inv_ntt(uint64_t *data) {
    inv_ntt(data, invomegas, shoupinvomegas, invpolyDegree,
            getModulus(_indexModulus));

    for (size_t idx = 0; idx < _degree; idx++) {
      data[idx] =
          mulmod{}(data[idx], invpoly_times_invphis[idx], _indexModulus);
    }
  }

  /** Performs an in-place number theoretic transform
   * @param data coefficients of the polynomial to be evaluated
   */
