...
// general timeout
constexpr auto receive_timeout = 15min;
...
expected<Unit> participateHelper(ZeroMQClientTransport& transport, SocketId socketId, std::string localIpAddr, bool passive) {
    ...
    if ((!passive)&&(statement == Statements::RSACeremony)) {
        ...
        zksnark::Prover<FieldT> prover(localStatement, irs, t, l, fieldGenerator, stringGenerator, mtParams, pdata, spdata, sdata, permut<k>::compute, permut<n>::compute, transport, hashedPublicData);
    ...
    if (hasError(result)) {
        LOG(INFO) << "Party failed with error message: " << showError(getError(result));
        exit(EXIT_FAILURE);
    }        
    ...
    if (passive) {
        ...
        std::string hashedPublicData = hash::hashPublicData(stringGenerator, pdata, spdata);
        ...
        mpz_class alpha = math::generateRandomValue(mpz_class(std::random_device()()), 210);
        uint64_t W1 = ligero::math::scaleToPrime(math::generateRandomValue(mpz_class(std::random_device()()), 62).get_ui(),0);
        uint64_t W2 = ligero::math::scaleToPrime(math::generateRandomValue(mpz_class(std::random_device()()), 62).get_ui(),0);        
        ...
        // Preliminary checks
        {
            ...
            // CRT deconstruction
            alpha_CRT = math::crt_deconstruct(alpha*mpz_class(params<uint64_t>::P[0]), mods);
            LOG(INFO) << "CRT 1:" << alpha_CRT[0];
            LOG(INFO) << "CRT 2:" << alpha_CRT[1];

            // CRT reconstruction
            std::vector<mpz_class> coefs;
            mpz_class reconstructed_alpha = math::crt_reconstruct(alpha_CRT, coefs, mods);

            assert(math::mod(reconstructed_alpha - alpha*mpz_class(params<uint64_t>::P[0]), mpz_class(params<uint64_t>::P[0])) == mpz_class(0));
            LOG(INFO) << math::mod(reconstructed_alpha - alpha*mpz_class(params<uint64_t>::P[0]), mpz_class(params<uint64_t>::P[0]));
 ...
 *  as the socket usage.
 */
expected<Unit> participate (std::string ipAddr, std::string localIpAddr, int numBits, int numCandidates, bool passive) {
    try {

        // RSA Ceremony
        SocketId socketId = boost::uuids::random_generator()();
        ZeroMQClientTransport transport(std::string("tcp://") + ipAddr + std::string(":5555"), socketId, receive_timeout);

        auto proceed = transport.joinThroughputTest(wait_timeout, timeout, data_size, nb_max_send);
...
int main(int argc, char** argv)
{
    ...
    auto result = participate(ipAddr, localIpAddr, bitsize, numCandidates, passive);
    if (hasError(result)) {
        return 1;
    }
    return 0;
