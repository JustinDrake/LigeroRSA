...
expected<Unit> verify(std::string basename) {
    ...
    fetchFromFile<SigmaProtocolPublicData>(spdata, basename+std::string(".sigmadata")); 
    ...
    for (size_t modulusIdx = 0; modulusIdx < q; modulusIdx++) {
        ...
        fetchFromFile<ligero::zksnark::FullFSTranscript<FieldT>>(transcript, basename+std::string(".proof." + std::to_string(modulusIdx))); 
...
int main(int argc, char** argv)
{
    ...
    try {
        if (hasError(verify(proof))) {
            std::cout << "Failed to process the proofs, exiting now." << std::endl;
            return EXIT_FAILURE;
        }
    } catch (...) {
        std::cout << "Failed to process the proofs, exiting now." << std::endl;
