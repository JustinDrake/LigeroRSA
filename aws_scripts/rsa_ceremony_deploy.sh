...
if [ -f "AWS/$EC2_KEY_ID.pem" ]; then
    ...
    # if we are fastracking, then wait for the binaries
    if [ "$FASTTRACK" = true ]; then
        echo
        echo "Provisioning done, awaiting binaries"
        echo "-----------------------------------"
        while [ ! -f "$BUILD_DIR/$COORDINATOR_BINARY" ] || [ ! -f "$BUILD_DIR/$PARTY_BINARY" ]; do
            sleep 1
        done
        echo
        echo "Binaries detected, resuming"
        echo "---------------------------"
    fi

    # Dispatch binaries
    ./rsa_dispatch_binaries.sh $((NBTOTALINSTANCES-1))
